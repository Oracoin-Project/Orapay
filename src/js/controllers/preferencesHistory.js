  ...
  function($scope, $log, $timeout, storageService, go, profileService, lodash) {
    ...
    $scope.csvHistory = function(cb) {
      ...
      getHistory(function(err, txs) {
        ...
        $scope.csvContent = [];
        $scope.csvFilename = 'Orapay-' + ($scope.alias || $scope.walletName) + '.csv';
        $scope.csvHeader = ['Date', 'Destination', 'Description', 'Amount', 'Currency', 'Txid', 'Creator', 'Orapayers', 'Comment'];
        ...
        data.forEach(function(it, index) {
          ...
          if (it.fees && (it.action == 'moved' || it.action == 'sent')) {
            ...
            $scope.csvContent.push({
              'Date': formatDate(it.time * 1000),
              'Destination': 'Oracoin Network Fees',
              'Description': '',
              'Amount': '-' + _fee,
              'Currency': 'ORA',
              'Txid': '',
