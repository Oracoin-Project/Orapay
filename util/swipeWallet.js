...
program
  ...
  .option('-f, --fee [n]', 'Optional, fee in ORA (default 0.0001 ORA), only if amount is not provided', parseFloat)
  .parse(process.argv);
        ...
        async.eachSeries(signers, function(dummy, cb) {
          ...
          function(err) {
            ...
            if (p.builder.isFullySigned()) {
              ...
              firstWallet.broadcastToOracoinNetwork(ntxid, function(txid) {
                console.log('\t #######  SENT  TXID:', txid);
                process.exit(1);
              });
            } else {
